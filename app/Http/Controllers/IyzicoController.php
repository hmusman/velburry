    ...
    public function callback(Request $request, $payment_type, $payment_data, $order_id){
        ...
        if ($payWithIyzico->getStatus() == 'success') {
            ...
            elseif ($payment_type == 'wallet_payment') {
                ...
                return $walletController->wallet_payment_done(json_decode($payment_data), $payment);
            }
            elseif ($payment_type == 'customer_package_payment') {
                $payment = $payWithIyzico->getRawResult();

                $customer_package_controller = new CustomerPackageController;
                return $customer_package_controller->purchase_payment_done(json_decode($payment_data), $payment);
            }
            elseif ($payment_type == 'seller_package_payment') {
                $payment = $payWithIyzico->getRawResult();

                $seller_package_controller = new SellerPackageController;
                return $seller_package_controller->purchase_payment_done(json_decode($payment_data), $payment);
            }
            else {
                dd($payment_type);
            }
